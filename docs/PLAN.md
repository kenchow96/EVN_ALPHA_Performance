...
| 7 — Motion engine | 🟠 Audit repair HITL in progress — Core 1 direct SRAM timer, anti-windup, 5 ms observer cadence, 25 kHz PWM, 12-gate metrics, and fresh battery preflight are verified. Large has a provisional compliant profile margin. CDC sweeping is retired after a reproducible fourth-case wedge. A build-verified, CRC-protected, flash-backed 16-case Medium sweep now runs without CDC, aborts on battery/RT failures, and enters BOOTSEL for UF2 extraction. Deployment/extraction is the active checkpoint; follow [ASSUMPTIONS.md](ASSUMPTIONS.md). **Phase 8 remains blocked** until all four motors pass the battery-gated profile suite and beat baseline. | 2026-09-02 | `909ac01`, `58c368c` + autonomous build |
