...
| 7 — Motion engine | 🟠 Audit repair HITL in progress — Core 1 timing, battery gates, 12 metrics, and autonomous flash extraction are verified. The startup reference governor is retained: matched A/B reduced positive acceleration 34%, jerk 52%, max error 21%, ripple 75%, and peak current 40%, with all positive overshoot <=0.5 deg. Remaining low-speed deceleration error points to under-modeled wheel friction; a build-verified 0.5x-2.0x friction-feedforward sweep is ready. Follow [ASSUMPTIONS.md](ASSUMPTIONS.md). **Phase 8 remains blocked** until all four motors pass the battery-gated profile suite and beat baseline. | 2026-09-02 | through `c2a4cb4` + friction sweep build |
