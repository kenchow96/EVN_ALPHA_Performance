...
| 7 — Motion engine | 🟠 Audit repair HITL in progress — Core 1 direct SRAM timer, anti-windup, 5 ms observer cadence, 25 kHz PWM, 12-gate metrics, and fresh battery preflight are verified. CDC sweeping is retired. Flash-backed tuning now passes 16/16 commit/trace CRCs; the timer repair holds resumed cases at 1000-1000 us with zero misses. Medium `W40/K5` is best at 10/12 but misses acceleration/tracking, and 0.55 start duty is intermittently insufficient. A build-verified four-repeat trapezoid/minimum-jerk A/B with restored 0.65 ramp is ready to deploy. Follow [ASSUMPTIONS.md](ASSUMPTIONS.md). **Phase 8 remains blocked** until all four motors pass the battery-gated profile suite and beat baseline. | 2026-09-02 | through `6618d0f` + trajectory A/B build |
