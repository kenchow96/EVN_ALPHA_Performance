...
| 7 — Motion engine | 🟠 Audit repair HITL in progress — Core 1 direct SRAM timer, anti-windup, 5 ms observer cadence, 25 kHz PWM, 12-gate metrics, fresh battery preflight, and autonomous flash extraction are verified. A 4x bidirectional A/B retained minimum-jerk as optional: it modestly reduced acceleration/jerk but did not improve gate count and regressed positive settling/ripple. Traces isolate the Medium failure to breakaway getting ahead of the paused reference, followed by reverse duty and ~4 deg catch-up error. A bounded startup reference governor is next. Follow [ASSUMPTIONS.md](ASSUMPTIONS.md). **Phase 8 remains blocked** until all four motors pass the battery-gated profile suite and beat baseline. | 2026-09-02 | through `fc7aa91` + trajectory A/B data |
