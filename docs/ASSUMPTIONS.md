...
| B6 | TinyUSB has exactly one execution owner | `CMakeLists.txt`, `EVN_ALPHA_Performance.c` | SDK worker owns `tud_task`; application drains its bounded queue only through SDK stdio | Ten full trace dumps on one handle + close/reopen/status | ✅ bulk USB stress pass |
