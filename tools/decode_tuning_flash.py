...
def decode_header(page, case_index, run_id):
    ...
    header = {
        ...
        "startup_pulse_on_ticks": read_u32(page, 49),
    }
...
def case_name(header):
    ...
    if header["run_id"] == START_DUTY_RUN_ID:
        ...
        direction = "pos" if header["delta_mdeg"] >= 0 else "neg"
        return f"S{duty}_R{header['repeat_index']}_{direction}"
    if header["run_id"] == RAMP_VALIDATION_RERUN_ID:
        direction = "pos" if header["delta_mdeg"] >= 0 else "neg"
        return f"Q{header['repeat_index']}_{direction}"
    ...
    if header["run_id"] == STARTUP_RAMP_RUN_ID:
        ...
        return (f"R{header['startup_ramp_ms']}_"
                f"N{header['repeat_index']}_{direction}")
        ...
        if header["status"] == STATUS_COMPLETE:
            ...
            metrics.update({
                "case": {
                    ...
                    "startup_pulse_on_ticks": header["startup_pulse_on_ticks"],
                },
                "battery_pre_run": {
